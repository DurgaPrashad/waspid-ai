...
import waspid.app_server.event_callback  # noqa: F401
from waspid.agent_server.env_parser import from_env
from waspid.app_server.app_conversation.app_conversation_info_service import (
    AppConversationInfoService,
    AppConversationInfoServiceInjector,
)
from waspid.app_server.app_conversation.app_conversation_service import (
    AppConversationService,
    AppConversationServiceInjector,
)
from waspid.app_server.app_conversation.app_conversation_start_task_service import (
    AppConversationStartTaskService,
    AppConversationStartTaskServiceInjector,
)
from waspid.app_server.app_lifespan.app_lifespan_service import AppLifespanService
from waspid.app_server.app_lifespan.oss_app_lifespan_service import (
    OssAppLifespanService,
)
from waspid.app_server.config_api.config_models import AppMode
from waspid.app_server.config_api.llm_model_service import (
    LLMModelService,
    LLMModelServiceInjector,
)
from waspid.app_server.event.event_service import EventService, EventServiceInjector
from waspid.app_server.event_callback.event_callback_service import (
...
from waspid.app_server.sandbox.sandbox_service import (
    SandboxService,
...
from waspid.app_server.services.db_session_injector import (
    DbSessionInjector,
)
from waspid.app_server.services.httpx_client_injector import HttpxClientInjector
from waspid.app_server.services.injector import InjectorState
from waspid.app_server.services.jwt_service import JwtService, JwtServiceInjector
from waspid.app_server.user.user_context import UserContext, UserContextInjector
from waspid.app_server.utils.environment import StorageProvider, get_storage_provider
from waspid.app_server.web_client.default_web_client_config_injector import (
    DefaultWebClientConfigInjector,
)
from waspid.app_server.web_client.web_client_config_injector import (
    WebClientConfigInjector,
)
from waspid.sdk.utils.models import WaspidModel
...
def get_waspid_provider_base_url() -> str | None:
    """Return the base URL for the Waspid provider, if configured.

    Falls back to LLM_BASE_URL for backward compatibility.
    """
    return os.getenv('WASPID_PROVIDER_BASE_URL') or os.getenv('LLM_BASE_URL') or None
...
# The SDK auto-fills this URL as the default for waspid/ and litellm_proxy/
# models.  Deployments (e.g. staging) may use a different LLM proxy, configured
# via WASPID_PROVIDER_BASE_URL.
...
def resolve_provider_llm_base_url(
    ...
    When the model uses ``waspid/`` or ``litellm_proxy/`` prefix and the
    ...
    Args:
        ...
        provider_base_url: Deployment provider URL.  Falls back to
            ``get_waspid_provider_base_url()`` when *None*.
    """
    if not model or not (
        model.startswith('waspid/') or model.startswith('litellm_proxy/')
    ):
        return base_url
    ...
    if provider_base_url is None:
        provider_base_url = get_waspid_provider_base_url()
...
def _get_default_lifespan():
    ...
    if 'saas' in (os.getenv('WASPID_CONFIG_CLS') or '').lower():
...
class AppServerConfig(WaspidModel):
    ...
    web_url: str | None = Field(
        ...
        description='The URL where Waspid is running (e.g., http://localhost:3000)',
    )
    ...
    waspid_provider_base_url: str | None = Field(
        default_factory=get_waspid_provider_base_url,
        description='Base URL for the Waspid managed LLM provider',
    )
    ...
    app_mode: AppMode = AppMode.WASPID
...
def config_from_env() -> AppServerConfig:
    # Import defaults...
    from waspid.app_server.app_conversation.live_status_app_conversation_service import (  # noqa: E501
        LiveStatusAppConversationServiceInjector,
    )
    from waspid.app_server.app_conversation.sql_app_conversation_info_service import (  # noqa: E501
        SQLAppConversationInfoServiceInjector,
    )
    from waspid.app_server.app_conversation.sql_app_conversation_start_task_service import (  # noqa: E501
        SQLAppConversationStartTaskServiceInjector,
    )
    from waspid.app_server.event.aws_event_service import (
        AwsEventServiceInjector,
    )
    from waspid.app_server.event.filesystem_event_service import (
    ...
    from waspid.app_server.event_callback.sql_event_callback_service import (
        SQLEventCallbackServiceInjector,
    )
    from waspid.app_server.sandbox.docker_sandbox_service import (
        DockerSandboxServiceInjector,
    )
    from waspid.app_server.sandbox.docker_sandbox_spec_service import (
        DockerSandboxSpecServiceInjector,
    )
    from waspid.app_server.sandbox.process_sandbox_service import (
        ProcessSandboxServiceInjector,
    )
    from waspid.app_server.sandbox.process_sandbox_spec_service import (
    ...
    from waspid.app_server.sandbox.remote_sandbox_spec_service import (
        RemoteSandboxSpecServiceInjector,
    )
    from waspid.app_server.user.auth_user_context import (
