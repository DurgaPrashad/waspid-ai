...
from waspid.app_server.integrations.service_types import ProviderType, SuggestedTask
...
from waspid.app_server.settings.settings_models import SandboxGroupingStrategy
from waspid.sdk.conversation import ConversationExecutionStatus
from waspid.sdk.llm import MetricsSnapshot
...
class AppConversationInfo(BaseModel):
    ...
    agent_kind: str = 'waspid'

    metrics: MetricsSnapshot | None = None

    parent_conversation_id: WaspidUUID | None = None
    sub_conversation_ids: list[WaspidUUID] = Field(default_factory=list)
...
class AppConversationStartRequest(WaspidModel):
...
class AppConversationStartTask(WaspidModel):
    ...
    app_conversation_id: WaspidUUID | None = Field(
...
class AppConversationStartTaskPage(WaspidModel):
