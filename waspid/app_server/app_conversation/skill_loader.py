...
async def _determine_org_repo_path(
    ...
    Examples:
        - GitHub/Bitbucket: ('owner/.waspid', 'owner')
        - GitLab: ('owner/waspid-config', 'owner')
        - Azure DevOps: ('org/waspid-config/waspid-config', 'org')
    """
    ...
    if is_gitlab:
        org_waspid_repo = f'{org_name}/waspid-config'
    elif is_azure_devops:
        org_waspid_repo = f'{org_name}/waspid-config/waspid-config'
    else:
        org_waspid_repo = f'{org_name}/.waspid'

    return org_waspid_repo, org_name


async def _get_org_repository_url(
    org_waspid_repo: str, user_context: UserContext
) -> str | None:
    """Get authenticated Git URL for organization repository.

    Args:
        org_waspid_repo: Organization repository path
    ...
    try:
        remote_url = await user_context.get_authenticated_git_url(
            org_waspid_repo, is_optional=True
        )
        return remote_url
    except AuthenticationError as e:
        _logger.debug(
            f'org-level skill directory {org_waspid_repo} not found: {str(e)}'
        )
        return None
    except Exception as e:
        _logger.debug(
            f'Failed to get authenticated URL for {org_waspid_repo}: {str(e)}'
        )
        return None
...
async def build_org_config(
    ...
    try:
        org_waspid_repo, org_name = await _determine_org_repo_path(
            selected_repository, user_context
        )

        org_repo_url = await _get_org_repository_url(org_waspid_repo, user_context)
        if not org_repo_url:
            return None
