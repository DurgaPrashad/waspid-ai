...
from waspid.sdk import Agent, LLMSummarizingCondenser
from waspid.sdk.context import AgentContext
from waspid.sdk.llm import LLM
from waspid.sdk.security import (
    AlwaysConfirm,
...
def get_project_dir(
    ...
    `.waspid/` configuration (setup.sh, pre-commit.sh, skills/, etc.).
    ...
    - .waspid/setup.sh execution
    - .waspid/pre-commit.sh (git hooks setup)
    - .waspid/skills/ (project skills)
    - PLAN.md path

    Args:
        working_dir: Base working directory path in the sandbox
            (e.g., '/workspace/project' from sandbox_spec)
        selected_repository: Repository name (e.g., 'Waspid/software-agent-sdk')
...
class AppConversationServiceBase(AppConversationService, ABC):
    ...
    async def load_and_merge_all_skills(
        ...
        - Public skills (from Waspid/skills GitHub repo)
        - User skills (from ~/.waspid/skills/)
        - Organization skills (from {org}/.waspid repo)
        - Project/repo skills (from repo .agents/skills/, .waspid/microagents/, and legacy .waspid/skills/)
    ...
    async def run_setup_scripts(
        ...
        # for all .waspid/ features (setup.sh, pre-commit.sh, skills).
    ...
    async def clone_or_init_git_repo(
        ...
        if request.selected_branch:
            ...
            waspid_workspace_branch = f'waspid-workspace-{random_str}'
            checkout_command = (
                f'git checkout -b {shlex.quote(waspid_workspace_branch)}'
            )
    ...
    async def maybe_setup_git_hooks(
        ...
        """Set up git hooks if .waspid/pre-commit.sh exists in the project root.

        Args:
            workspace: Remote workspace for command execution.
            project_dir: Project root directory (repo root when a repo is selected).
        """
        command = 'mkdir -p .git/hooks && chmod +x .waspid/pre-commit.sh'
        ...
        with tempfile.TemporaryFile(mode='w+t') as temp_file:
            ...
            if download_result.success:
                ...
                if 'This hook was installed by Waspid' not in temp_file.read():
