...
from waspid.agent_server.models import Success
...
from waspid.app_server.services.db_session_injector import set_db_session_keep_open
from waspid.app_server.services.httpx_client_injector import (
...
from waspid.app_server.user_auth import get_user_settings
from waspid.app_server.utils.dependencies import get_dependencies
from waspid.app_server.utils.docker_utils import (
    replace_localhost_hostname_for_docker,
)
from waspid.app_server.utils.llm import resolve_llm_base_url
from waspid.sdk.skills import KeywordTrigger, TaskTrigger
...
async def _get_agent_server_context(
    ...
    if not sandbox_spec:
        ...
        # sandbox spec versions when updating Waspid. When the SandboxSpecServices
...
async def switch_conversation_profile(
    ...
    # picks up the provider default (e.g. the Waspid LiteLLM proxy).
...
async def get_conversation_skills(
    ...
    - Global skills (Waspid/skills/)
    - User skills (~/.waspid/skills/)
    - Organization skills (org/.waspid repository)
    - Repository skills (repo .agents/skills/, .waspid/microagents/, and legacy .waspid/skills/)
...
async def get_conversation_hooks(
    ...
    workspace (i.e. `{project_dir}/.waspid/hooks.json`) at request time.

    Note:
        This is intentionally a "live" view of the workspace configuration.
        If `.waspid/hooks.json` changes over time, this endpoint reflects the
