...
from waspid.app_server.sandbox.docker_sandbox_service import DockerSandboxService
from waspid.app_server.sandbox.sandbox_models import (
    AGENT_SERVER,
    SandboxInfo,
    SandboxStatus,
)
from waspid.app_server.sandbox.sandbox_service import SandboxService
from waspid.app_server.sandbox.sandbox_spec_service import SandboxSpecService
...
from waspid.app_server.utils.git import ensure_valid_git_branch_name
from waspid.app_server.utils.llm_metadata import (
    get_llm_metadata,
    should_set_litellm_extra_body,
)
from waspid.sdk import Agent, AgentContext, LocalWorkspace
from waspid.sdk.hooks import HookConfig
from waspid.sdk.llm import LLM
from waspid.sdk.plugin import PluginSource
from waspid.sdk.secret import LookupSecret, StaticSecret
from waspid.sdk.settings import ACPAgentSettings
from waspid.sdk.subagent import get_registered_agent_definitions
from waspid.sdk.utils.paging import page_iterator
from waspid.sdk.utils.redact import (
    redact_api_key_literals,
    redact_text_secrets,
    sanitize_config,
)
from waspid.sdk.workspace.remote.async_remote_workspace import AsyncRemoteWorkspace
from waspid.tools.preset.default import (
    get_default_tools,
    register_builtins_agents,
)
from waspid.tools.preset.planning import (
...
class LiveStatusAppConversationService(AppConversationServiceBase):
    ...
    web_url: str | None
    waspid_provider_base_url: str | None
    ...
    def _configure_llm(self, user: UserInfo, llm_model: str | None) -> LLM:
        ...
        base_url = resolve_provider_llm_base_url(
            ...
            provider_base_url=self.waspid_provider_base_url,
        )
    ...
    async def _add_system_mcp_servers(
        ...
        """Add system-generated MCP servers (default Waspid server).
        ...
        Args:
            ...
            conversation_id: Conversation ID forwarded to the Waspid MCP server
        """
        if not self.web_url:
            return

        # Add default Waspid MCP server (includes Tavily proxy if configured)
        mcp_url = f'{self.web_url}/mcp/mcp'
        mcp_servers['default'] = {
            'url': mcp_url,
            'headers': {'X-Waspid-ServerConversation-ID': str(conversation_id)},
        }
    ...
    def _apply_server_agent_overrides(
        ...
        * LLM tracing metadata for SaaS analytics.
        ...
        # LLM tracing metadata for waspid/ models
    ...
    async def _load_hooks_from_workspace(
        ...
        """Load hooks from .waspid/hooks.json in the remote workspace.

        This enables project-level hooks to be automatically loaded when starting
        a conversation, similar to how Waspid-CLI loads hooks from the workspace.
    ...
    async def _build_start_conversation_request_for_user(
        ...
        if api_secrets:
            from waspid.app_server.constants import (
    ...
    async def _process_pending_messages(
        ...
        # The frontend uses task-{uuid.hex} format (no hyphens), matching WaspidUUID serialization
    ...
    async def inject(
        ...
        async with (
            ...
            yield LiveStatusAppConversationService(
                ...
                web_url=web_url,
                waspid_provider_base_url=config.waspid_provider_base_url,
