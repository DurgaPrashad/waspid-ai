...
def _post_merge_llm_fixups(settings: Settings) -> None:
    ...
    rules to :func:`waspid.app_server.utils.llm.resolve_llm_base_url` so the
    personal-save and enterprise org-defaults paths stay in lockstep.
    """
    if not isinstance(settings.agent_settings, WaspidAgentSettings):
        return
...
async def load_settings(
    ...
    try:
        ...
        # Convert litellm_proxy/ back to waspid/ for the frontend, but only
        # if the base_url is the Waspid proxy. Custom litellm_proxy endpoints
