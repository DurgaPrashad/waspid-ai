...
- Settings: Persisted settings for Waspid sessions
...
from waspid.app_server.integrations.provider import ProviderToken
...
def _load_persisted_agent_settings(
    data: Any,
) -> WaspidAgentSettings | ACPAgentSettings:
...
class Settings(BaseModel):
    """Persisted settings for Waspid sessions.
    ...
    def update(self, payload: dict[str, Any]) -> None:
        ...
        if isinstance(agent_update, dict):
            ...
            if new_kind and new_kind != current_kind:
                ...
                # ``WaspidAgentSettings | ACPAgentSettings``. Deep-merging
    ...
    def agent_settings_serializer(
        self,
        agent_settings: WaspidAgentSettings | ACPAgentSettings,
    ...
    def _normalize_inputs(cls, data: dict | object) -> dict | object:
        ...
        elif isinstance(agent_settings, (WaspidAgentSettings, ACPAgentSettings)):
    ...
    def to_agent_settings(self) -> WaspidAgentSettings | ACPAgentSettings:
        return self.agent_settings

    def get_agent_settings_display(self) -> dict[str, Any]:
        """Return agent_settings dict with display-friendly model names.

        ``litellm_proxy/`` prefixes are normalised to ``waspid/`` only when
        using the Waspid proxy URL. Custom litellm_proxy endpoints keep their
        ...
        if isinstance(llm, dict):
            ...
            # Only convert litellm_proxy/ to waspid/ if using the Waspid proxy
            if isinstance(model, str) and model.startswith('litellm_proxy/'):
                ...
                if normalized_base == normalized_proxy:
                    llm['model'] = f'waspid/{model.removeprefix("litellm_proxy/")}'
