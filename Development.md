...
Waspid supports a diverse array of Language Models (LMs) through the powerful [litellm](https://docs.litellm.ai) library.

For the V1 web app, start Waspid and configure your model and API key in the Settings UI.
...
**Important:** Keep your project files in the WSL filesystem (e.g., `~/workspace/waspid`), not in `/mnt/c`. Files accessed via `/mnt/c` will be significantly slower.
...
If you just want to run Waspid without setting up a dev environment:
...
## Running Waspid with Waspid

You can use Waspid to develop and improve Waspid itself.
