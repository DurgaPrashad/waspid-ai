...
WASPID_BASE_URL = os.environ.get('WASPID_BASE_URL', 'https://app.all-hands.dev')
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            'Start an Waspid Cloud conversation that checks a GitHub issue '
            'for duplicates.'
        )
    )
    parser.add_argument(
...
def github_headers() -> dict[str, str]:
    headers = {
        ...
        'User-Agent': 'waspid-issue-duplicate-check',
...
def waspid_headers() -> dict[str, str]:
    api_key = os.environ.get('WASPID_API_KEY')
    if not api_key:
        raise RuntimeError('WASPID_API_KEY environment variable is required')
    return {
...
def poll_start_task(
    ...
    while time.time() < deadline:
        payload = request_json(
            ...
            headers={'Authorization': waspid_headers()['Authorization']},
        )
        ...
        if status in {'ERROR', 'FAILED'}:
            raise RuntimeError(f'Waspid start task failed: {json.dumps(item)}')
        time.sleep(poll_interval_seconds)
    raise TimeoutError(
...
def poll_conversation(
    ...
    while time.time() < deadline:
        ...
        if execution_status in FAILED_EXECUTION_STATUSES:
            raise RuntimeError(
                'Waspid conversation ended with '
...
def main() -> int:
    ...
    conversation_url = (
        ...
        or f'{WASPID_BASE_URL}/conversations/{app_conversation_id}'
    )
    ...
    if session_api_key_value and not isinstance(session_api_key_value, str):
        raise RuntimeError(
            'session_api_key had unexpected type in the Waspid conversation: '
        ...
        try:
            ...
            if not session_api_key:
                raise RuntimeError(
                    ...
                    'session_api_key was missing from the Waspid conversation'
                ) from exc
