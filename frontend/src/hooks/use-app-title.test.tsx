...
describe("useAppTitle", () => {
  ...
  it("should return 'Waspid' if is OSS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() => expect(result.current).toBe("Waspid"));
  });

  it("should return 'Waspid Cloud' if is SaaS and NOT in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    getConfigSpy.mockResolvedValue({
      app_mode: "saas",
    });
    ...
    await waitFor(() => expect(result.current).toBe("Waspid Cloud"));
  });

  it("should return '{some title} | Waspid' if is OSS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
    ...
    await waitFor(() =>
      expect(result.current).toBe("My Conversation | Waspid"),
    );
  });

  it("should return '{some title} | Waspid Cloud' if is SaaS and in /conversations", async () => {
    // @ts-expect-error - only returning partial config for test
