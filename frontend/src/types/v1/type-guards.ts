...
import type { WaspidParsedEvent } from "../core/index";
 ...
 */
export const isMessageEvent = (event: WaspidEvent): event is MessageEvent =>
 ...
 */
export const isActionEvent = (event: WaspidEvent): event is ActionEvent =>
  event.source === "agent" &&
  "action" in event &&
  event.action !== null &&
 ...
 */
export const isDisplayableErrorEvent = (event: WaspidEvent): boolean =>
...
/**
 * TEMPORARY: Type guard to check if an event is a V1 WaspidEvent
...
export function isV1Event(
  event: WaspidEvent | WaspidParsedEvent,
): event is WaspidEvent {
...
/**
 * TEMPORARY: Type guard to check if an event is a V0 WaspidParsedEvent
 *
 * @deprecated This is temporary until full V1 migration is complete
 */
